...
A function declaration consists of a \verb!BeginFn FnName!, followed by the function label prefixed with an underscore: \verb!_FnName!. An \verb!InitFrame! instruction is then required (with the number of the function locals): \verb!InitFrame 10!. All parameters must now be ``popped" in separate \verb!PushParam! instructions. If the function takes no parameters then no \verb!PushParam! instructions are required.
