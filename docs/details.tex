...
In both cases (with and without an \verb!ELSE!) the true portion of the \verb!IF! statement is now translated. We generate a \verb!TT_LABEL! denoting the start of the true section (as we discussed earlier: \verb!__ifXtrue!). The true branch is now passed to \verb!make_simple!. Finally, to complete the \verb!IF! statement, the aforementioned end label is appended.
	...
	\item[Functions] Each function declaration generates a minimum of 6 TAC instructions, these are detailed in the relevant sections below.
		\begin{description}
			\item[D]
			\item[d]  %*************************************** HERE HERE HERE ********************************************
			\item[F]
		\end{description}
...
By the time the \verb!make_simple! call returns, a linked list of complete \verb!tac_quad! structures has been created that constitutes the translation of the user \mcc program into TAC representation. As mentioned previously, these structures do not contain strings or lexemes, but use pointers in to an environment structure. %This was the suggested approach in [QUOTE DRAGON BOOK HERE]
%ADDITIONALLY, another benefit was that a lot of the functionality, such as type checking and a working scope system could be inherited from the interpreter. (ENVIRONMENT)
