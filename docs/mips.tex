...
\ \\ \ \\
In most languages, the Activation Record can be simply placed on the stack and popped off when the function terminates. There are generally two pointers into the stack, a stack pointer (conveniently held in register \$sp) and a frame pointer (\$fp). The stack pointer points to the top of the stack and the frame pointer is typically set to the end of the current activation record. This makes it easy to create ``well-known" places within the Activation Record where certain, specific information can be stored.
\ \\ \ \\
