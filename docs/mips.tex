...
At runtime we do not have full-access to the type of the information that was available during compilation time. We only have access to the information that we specifically placed in registers or in memory. Typically, a lot of compilers will make use of ``the stack" to store useful information that is needed as the program is executed. The stack implementation is provided by the MIPS architecture and we are able to push / pop values at will. The stack is a good place to push information when a branch statement is encountered, as it can easily be restored by popping the values back off. For each function that is invoked, a structure is (generally) created called an ``Activation Record". The purpose of an activation record is to have a standardised place where locals and other function specific runtime information can be stored. When the local variables are in registers and a function call happens, the modified locals can be persisted into the Activation Record and easily restored when the function returns.
\ \\ \ \\
In most languages, the Activation Record can simply be placed on the stack and popped off when the function terminates. There are generally two pointers into the stack, a stack pointer (conveniently held in register \$sp) and a frame pointer (\$fp). The stack pointer points to the top of the stack and the frame pointer is typically set to the end of the current activation record. This makes it easy to create ``well-known" places within the Activation Record where certain, specific information can be stored. With these two pointers we are able to index into the middle of the Activation Record from either end.
\ \\ \ \\
...
\ \\ \ \\
The first implication, is that our Activation Record must contain a static link to the containing function's stack frame. This information is presented well with examples in Muchnick's book. The re-designed activation record in use can be seen in figure \ref{fig:AR}.
\begin{quotation}
\ \\
